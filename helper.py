...
class EncoderLayer(nn.Module):
  def __init__(self, d_model, num_heads, dff, rate=0.1):
    ...
    self.mha = MultiHeadAttention(d_model, num_heads).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
    self.ffn = cf.point_wise_feed_forward_network(d_model, dff)
...
class Encoder(nn.Module):
  def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size,
    ...
    self.pos_encoding = cf.positional_encoding(maximum_position_encoding ,d_model)
  ...
  def forward(self, x, mask):
    ...
    x = (x.cpu() + self.pos_encoding[:, :seq_len, :]).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
...
class TaggingTransformer(nn.Module):
  def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size,
    ...
    self.encoder = Encoder(num_layers, d_model, num_heads,dff, input_vocab_size,
                         pe_input, word_emb=word_emb).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
